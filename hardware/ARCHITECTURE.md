...
The design utilizes a **120L reservoir** with a **100L operating level**. This volume is calculated based on the "Rule of Thumb" that leafy greens require 2L of buffering per plant.
...
- **Micro-Dosing:** Stepper motors allow for µL range precision, which is critical because 100L of soft water may require as little as 0.2 mL of acid to shift the pH by 0.1 points.
...
- **Microcontroller:** while the chip offers modern connectivity (Matter/Thread), concurrency must be managed via software (RTOS) rather than physical core separation.
...
- 24V Actuators: Peak draw of ~5.3A (Main pump + two nutrient pumps + reflected logic load), leaving nearly 20% headroom on the 150W supply.
...
Requirements:
- Use PSU with built-in soft-start
...
**IP67 wet-environment alternative:** LPV-100-24 (24V / 4.2A / 100W).


Consider UPS/battery backup for main pump to **prevent plant stress during power outages**.
...
Choose a Fsw = 1MHz to balance efficiency and footprint. 1 MHz is considered the "sweet spot" for 24V-to-5V conversion. It provides a stable operating environment for the ESP32-C6 (that has its own LDO) and TF-Luna LiDAR, which are both powered by the 5V rail.
...
**Total 3.3V Rail** | **~100 mA** | **~150 mA** | **Regulator:** AMS1117 (1A Rated)
...
Reflected 3.3V Rail   |      100 mA |      150 mA | LDO => current in = current out
**Total 5V Rail**     |  **240 mA** |  **750 mA** | **Regulator:** TPS62933 (3A Rated)



#### 24V Main Power Rail Budget (Actuators)

The 24V rail powers all high-draw components. Dosing pumps are disabled when idle to save power. Per the dosing sequence, nutrients and pH Down never step simultaneously. The system is designed so that the three dosing pumps are disabled when idle to conserve power
...
Reflected 5V Rail           |      270 mA |      835 mA | assuming Buck at 90% efficiency
**Total 24V Rail**          |**3,670 mA** |**5,315 mA** | See peak load analysis below[^3]

[^3]: Worst-case is main pump running with both nutrient dosing pumps active.

**Peak Analysis:** The sources specify that the system should never run all dosing pumps  simultaneously with the main pump during normal operation
- Normal Worst-Case Scenario: The main circulation pump is running while one dosing pump is active
- Selected Power Supply: Mean Well LRS-150-24 (24V / 6.5A / 150W)
- Capacity Margin: With a peak actuator draw of ~5.315A and logic loads, the 6.5A supply provides ample headroom, ensuring the system remains stable even if multiple components cycle at once.
...
Bulk capacitance acts as a local energy reservoir — storing charge close to the load so it can be delivered instantly when current demand spikes, without that demand travelling all the way back to the power supply.

**The problem they solve:** Every wire and PCB trace has inductance. Inductance resists instantaneous changes in current: V = L × dI/dt. When a stepper driver or motor demands a sudden surge of current, the inductance of the supply path creates a voltage drop — the rail sags. The further the current has to travel from the supply, the worse the sag.

A bulk capacitor placed close to the load supplies that surge current locally and instantly, before the inductance of the supply path has time to cause a sag.

**Standard values by rail and place:**

Rail | Place               | Peak Current | Value / Voltage | Dielectric             | Purpose
----:|---------------------|--------------|----------------:|------------------------|--------
 24V | Main power entry    | ~5.3A        | 1000µF / 50V    | Aluminium electrolytic[^4] | Primary reservoir
 24V | Main power entry    | ~5.3A        |   10µF / 50V    | MLCC X7R[^5]           | Mid-frequency bridge
 24V | Each TMC2209 VM pin | ~1.5A        |  220µF / 50V    | Aluminium electrolytic | Local reservoir
 24V | Main Pump MOSFET    | ~1.2A        |  470µF / 50V    | Aluminium electrolytic | Local reservoir
  5V | Buck output         | ~0.75A       |  220µF / 10V    | Aluminium polymer[^6]  | ESP32 WiFi Tx
  5V | Buck output         | ~0.75A       |   10µF / 10V    | MLCC X7R[^7]           | Mid-frequency bridge
...
- If the acceptable ripple and transient duration are known, use the capacitor charge equation: C = I × Δt / ΔV instead. Otherwise, provide 100µF to 200µF electrolytic capacitors for every 1A of load current. 
- Use electrolytic capacitors that are rated for 150% to 200% of the expected voltage to reduce aging and provide transient headroom.
- Aluminium polymer is the lower ESR, but hard to find at above 25V. Aluminium electrolytic capacitors are a pragmatic choice for 50V.
- **Placement rule:** place the capacitors close to the load it is protecting.
- Both the Benewake TF-Luna LiDAR and the mechanical float switches include **100nF** high-frequency capacitors to debounce signals and prevent EMI-induced false triggers on the safety interlock lines.
...
### 7. Trace Width Requirements:


Rail | To           | Target Current            | Trace Width | Rationale
----:|--------------|---------------------------|-------------|----------
 24V | Entry        | 5.3A (Peak) to 6.5A (PSU) | 3.0mm       | Reduce voltage drop.
 24V | Main pump    | 1.2A (Continous)          | 1.0mm       | Manage heat
 24V | Dosing Pumps | 1.53A (Peak)              | 1.0mm       | Lower inductance
  5V | Peripherals  | 0.75A (Peak)              | 0.5mm       | Stable power
3.3V | Logic        | 0.15 (Peak)               | 0.3mm       | Minimal load
...
To ensure industrial-grade reliability and chemical stability for the OPNhydroponics system, the PCB layout must adhere to the following power integrity and EMI mitigation guidelines derived from the hardware architecture and schematic.

### 1.7 PCB Stackup

1. **Layer Stack and Copper Weight**
   - **4-Layer Design:** Use a 4-layer PCB to provide dedicated planes for power and ground, which is essential for managing EMI in a system with high-speed switching drivers.
   - **2oz Copper:** Specify **2oz copper for outer layers** to safely handle the **5.315A peak system current** without excessive heat generation or voltage drop.
   - **Size:** 100mm × 80mm (fits common enclosures)
   - **Finish:** HASL or ENIG

2. **Plane and Routing Guidelines**
To ensure the stability of sensitive probes and the microcontroller, the stackup should be utilized as follows:
   - **Solid Ground Plane:** A **dedicated solid ground plane** is required. This plane is vital for providing a clean return path and suppressing noise from the motor drivers.
...
2. **Sensor Isolation and Ground Loop Prevention**
   - **Physical Air Gap:** When laying out the **ADM3260 I2C isolators**, maintain a clear physical separation between the main system ground and the **isolated ground islands** for the pH and EC probes.
   - **Analog Separation:** Keep all analog traces for the pH, EC, and RTD probes physically distant from the switching power supplies to protect sensitive readings.
   - **I2C Traces:** Route away from 24V pump power traces if near pump power.


### Board Specifications


### 4. PCB Layout Guidelines for Power Integrity
